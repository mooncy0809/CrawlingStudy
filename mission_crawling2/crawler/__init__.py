...
class SeleniumRequest(object):
    ...
    def __init__(self,driver_path, options=None):
        ...
        self.driver = webdriver.Chrome( executable_path=self.driver_path, chrome_options=self.chrome_optins)

    def _make_chrome_options(self, options):
        if options is not None:
            self.options += options

        self.chrome_optins = Options()
        for opt in set(self.options):
            self.chrome_optins.add_argument( opt )
